...
configure_containerd() {
    ...
    if ! containerd config dump >/dev/null 2>&1; then
        log_warning "配置文件可能有问题，使用简化配置"
        create_minimal_containerd_config
    fi
    
    log_success "containerd 配置完成"
}

# 创建最小化的 containerd 配置
create_minimal_containerd_config() {
    log_info "创建简化的 containerd 配置..."
...
fix_containerd_cri() {
    ...
    if command -v crictl >/dev/null 2>&1; then
        if crictl version >/dev/null 2>&1; then
            log_success "CRI 连接正常"
            return 0
        else
            log_warning "CRI 连接失败，尝试更多修复..."
        fi
    fi
    
    # 如果还是失败，尝试重置 containerd
    log_info "重置 containerd 数据..."
    systemctl stop containerd
...
initialize_master() {
    ...
    log_step "预拉取镜像"
    if ! kubeadm config images pull --image-repository=registry.aliyuncs.com/google_containers --kubernetes-version=$K8S_VERSION; then
        log_warning "镜像预拉取失败，可能是 containerd CRI 配置问题"
        
        # 尝试修复 containerd CRI
        if fix_containerd_cri; then
            log_info "重新尝试拉取镜像..."
            if kubeadm config images pull --image-repository=registry.aliyuncs.com/google_containers --kubernetes-version=$K8S_VERSION; then
                log_success "镜像拉取成功"
            else
                log_warning "镜像拉取仍然失败，但将继续初始化（镜像会在初始化时自动拉取）"
            fi
        else
            log_warning "CRI 修复失败，但将继续初始化"
        fi
    fi
